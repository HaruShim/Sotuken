    ...
    def handle(self, *args, **options):
        ...
        with open(file_path, 'w') as file:
            ...
            for check in diff:
                l = check.split(',')
                writer.writerow(
                    [str(l[0]),l[1],str(l[2]),l[3],l[4],l[5],l[6],l[7],str(l[8]),str(l[9]),str(l[10]),])

        # 保存ディレクトリのファイルリストを取得
        files = os.listdir(settings.BACKUP_PATH_EMPLOYEEINFO_D)
