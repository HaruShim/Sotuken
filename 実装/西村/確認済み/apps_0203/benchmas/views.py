...
class S0501CPUView(ListView):
    ...
    def mylist(request):
        ...
        if q_word:
            ...
            # id降順でクエリセットを代入
            object_list = object_list.objects.filter(
                Q(cpu_name__icontains=q_word))
        if q_word2:
            object_list = object_list.objects.filter(
                Q(passmark__icontains=q_word2) |
                Q(core__icontains=q_word2) |
                Q(thread__icontains=q_word2) |
                Q(tdp__icontains=q_word2) |
                Q(rated_clock__icontains=q_word2) |
                Q(max_clock__icontains=q_word2) |
                Q(list_price__icontains=q_word2)|
                Q(cost_performance__icontains=q_word2))
...
class S0501GPUView(ListView):
    ...
    def mylist(request):
        ...
        if q_word:
            object_list = object_list.filter(Q(gpu_name__icontains=q_word))
        if q_word2:
            # object_listにBottleneckモデルからq_wordの検索結果
            # id降順でクエリセットを代入
            object_list = object_list.filter(Q(marktimespygraphics__icontains=q_word2) |
                                                Q(tdp__icontains=q_word2) |
                                                Q(cost_performance__icontains=q_word2) |
                                                Q(wattage_performance__icontains=q_word2) |
                                                Q(list_price__icontains=q_word2))
        paginator = Paginator(object_list, 8)
        page = request.GET.get('page', 1)
        try:
        ...
        # リクエスト元にtemplate_nameや、辞書型の変数を返す
        return render(request, template_name, context)
class S0502CPUView(LoginRequiredMixin, CreateView):
    """S0501View
...
class S0503CPUView(LoginRequiredMixin, UpdateView):
    """S0501View
...
class S0503GPUView(LoginRequiredMixin, UpdateView):
    """S0501View
