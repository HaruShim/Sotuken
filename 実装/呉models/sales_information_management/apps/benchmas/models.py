...
class GpuBench(models.Model):
    ...
    """
    id = models.AutoField(verbose_name='GPUベンチマークID',
                          db_column='gpu_bench_id', primary_key=True, editable=False)
    gpu_name = models.CharField(verbose_name='GPU名称', max_length=60)
    marktimespygraphics = models.IntegerField(
        verbose_name='Mark Time Spy Graphics')
...
class CpuBench(models.Model):
    ...
    """
    id = models.AutoField(verbose_name='CPUベンチマークID',
                          db_column='cpu_bench_id', primary_key=True, editable=False)
